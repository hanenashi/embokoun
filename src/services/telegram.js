...
(function() {
    ...
    function extractImageItems(doc, baseUrl) {
        ...
        doc.querySelectorAll('.tgme_widget_message_photo_wrap').forEach(photo => {
            ...
            uniqPush(items, { type: 'image', url: absUrl(bg, baseUrl), href: absUrl(href, baseUrl) });
        });
        ...
        doc.querySelectorAll('.tgme_widget_message_photo_wrap img[src], .tgme_widget_message_bubble img[src], img.tgme_widget_message_photo[src]').forEach(img => {
            uniqPush(items, { type: 'image', url: absUrl(img.getAttribute('src'), baseUrl), href: '' });
        });
    ...
    function tileBaseStyle(total) {
        return ['position:relative;', 'display:block;', 'overflow:hidden;', 'background:#111;', 'min-height:120px;', total === 1 ? 'aspect-ratio:16/9;' : 'aspect-ratio:1/1;'].join('');
    }
    ...
    function loadVideoIntoTile(tile, item, total) {
        ...
        const download = root.blob.download(item.url, {
            ...
            onProgress(progress) {
                ...
                status.textContent = totalBytes > 0
                    ? `Downloading video... ${root.blob.formatBytes(loaded)} / ${root.blob.formatBytes(totalBytes)}`
                    : `Downloading video... ${root.blob.formatBytes(loaded)}`;
            }
        });
        ...
        download.promise.then(blob => {
            ...
            const video = root.blob.makeVideoFromBlob(blob.blobUrl, { key: 'telegram-card', label: 'Telegram', style: `${total === 1 ? 'aspect-ratio:16/9;' : 'aspect-ratio:1/1;'}background:#000;box-shadow:none;border-radius:0;` }, () => mediaTile(item, 0, total));
    ...
    function appendMediaGrid(card, mediaItems) {
        ...
        grid.style.cssText = ['display:grid;', count === 1 ? 'grid-template-columns:1fr;' : 'grid-template-columns:repeat(2,1fr);', 'gap:2px;', 'background:#111;'].join('');
    ...
    function makeIframe(channel, postId) {
        return root.render.fallbackFrame({ src: `https://t.me/${encodeURIComponent(channel)}/${encodeURIComponent(postId)}?embed=1&mode=tme`, mode: 'normal', aspect: '4/5', reason: 'telegram-iframe' });
    }
    ...
    root.services.register({
        ...
        match(url) { return url.match(/(?:t\.me|telegram\.me)\/(?:s\/)?([a-zA-Z0-9_]+)\/(\d+)/i); },
        placeholderImage(ctx) {
            const channel = ctx.match[1];
            const postId = ctx.match[2];
            return fetchThumb(`https://t.me/${encodeURIComponent(channel)}/${encodeURIComponent(postId)}?embed=1&mode=tme`);
        },
        async resolve(ctx) {
            const channel = ctx.match[1];
            const postId = ctx.match[2];
            ...
            if (isPureSingleVideo) {
                ...
                return { kind: 'video-url', url: videoItems[0].url, referer: 'https://t.me/', aspect: '16/9', reason: 'telegram-video' };
            }
            if (mediaItems.length || textHtml) {
                root.log.info('telegram', 'resolved native card', channel, postId, `media=${mediaItems.length}`);
                return { kind: 'native-node', node: makeCard({ imageUrl: imageItems[0] && imageItems[0].url, videoUrl: videoItems[0] && videoItems[0].url, mediaItems, textHtml, author, meta, originalUrl: publicUrl }), reason: 'telegram-card' };
            }
            root.log.warn('telegram', 'no media/text found; iframe fallback', channel, postId);
            return { kind: 'iframe', url: embedUrl, widthMode: 'normal', aspect: '4/5', reason: 'telegram-iframe' };
        },
        fallback(ctx) { return makeIframe(ctx.match[1], ctx.match[2]); }
    });
})();
